...
def run_non_interactive(
    ...
    if currency and currency.upper() != "USD":
        ...
        rate = fetch_fx_rate("USD", target)
        if rate is not None and rate > 0:
            display_currency = target
            usd_per_target = rate
        else:
            print(
                f"Warning: could not fetch a USD→{target} exchange rate — "
                "falling back to USD.",
                file=sys.stderr,
            )
    ...
    for key, label, workload in scenarios:
        ...
        if usd_per_target is not None:
            # convert_rows_currency divides by a USD-per-target rate, so
            # pass the USD→target rate directly (e.g. USD→GBP ≈ 0.79).
    ...
    if multiple:
        print("\n== Results (all scenarios) ==")
        print(
            render_combined_table(scenario_labels_rows, currency=display_currency)
        )
    else:
        label, rows = scenario_labels_rows[0]
        print(f"\n== {label} ==")
